...
def main(args):
    ...
    generator = Generator(
        distributions, type_distribution, definitions, dummy_schema, DATA_URL
    )
    ...
    for interaction in interactions:
        ...
        if interaction["include"] and specified:
            ...
            for d in range(1, MAX_DIMENSIONS + 1):
                ...
                groups = []
                for _ in range(n):
                    specs = generator.generate_interaction(
                        interaction["props"], d, seen_base_specs, cross
                    )
                    ...
                    if tries == NUM_TRIES:
                        logger.warning(
                            "exceeded maximum tries for {0} with d={1}".format(
                                interaction["name"], d
                            )
                        )
                        continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir", "-o", default=absolute_path("../../data/to_label")
    )
